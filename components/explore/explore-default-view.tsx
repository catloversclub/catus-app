import PopularCatCard, { PopularCat } from "@/components/explore/popular-cat-card";
...
const ExploreDefaultView = ({ popularCats }: ExploreDefaultViewProps) => (
  <ScrollView
    className="flex-1"
  ...
  >
    <View className="px-3 pt-6 gap-6">
      <Pressable
        className="px-3 py-4 rounded gap-0.5 border border-[#C2E9FF] bg-[#EBF8FF]"
      >
    ...
    <View className="gap-3">
      ...
      <View className="px-3 gap-3">
        {popularCats.map((cat) => (
          <PopularCatCard key={cat.id} cat={cat} />
        ))}
      </View>
    </View>
  </ScrollView>
